...
export class NewArticleComponent implements OnInit {
  ...
  constructor(private articleService: ArticleService, private repositoryService: RepositoryService, private router: Router) {
    ...
    const x = articleService.startArticleProcess(this.user);
    x.subscribe(
      res => {
        console.log(res);
  ...
  onSubmit(value, form) {
    ...
    const x = this.articleService.postNewArticle(o, this.formFieldsDto.taskId);

    x.subscribe(
      res => {
        alert('predji sad na informacije o autoru');
        this.router.navigateByUrl('article-basic-info/' + this.processInstance);
      },
      err => {
        alert(err.error);
      }
