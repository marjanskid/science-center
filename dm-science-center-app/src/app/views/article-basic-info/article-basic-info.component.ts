...
@Component({
  selector: 'app-article-basic-info',
  templateUrl: './article-basic-info.component.html',
  styleUrls: ['./article-basic-info.component.css']
})
export class ArticleBasicInfoComponent implements OnInit {
  ...
  constructor(private articleService: ArticleService, private repositoryService: RepositoryService,
    ...
    this.processInstance = this.route.snapshot.params.processInstanceId;
  ...
  onSubmit(value, form) {
    ...
    const x = this.articleService.postArticleAuthorBasicInfo(o, this.formFieldsDto.taskId);

    x.subscribe(
      res => {
        alert('predji sad na informacije o autoru');
        this.router.navigateByUrl('article-coauthor-info/' + this.processInstance);
      },
      err => {
        alert(err.error);
      }
